...
test('prayer working/rarely soft-rejects; five_daily passes', () => {
  for (const p of ['working', 'rarely']) {
    assert.equal(
      evaluateGate({ ...GOOD_ANSWERS, prayer: p }, ADULT).result,
      'auto_reject'
    );
  }
});
...
test('E1/E2 not_affirm and E3 continuing soft-reject', () => {
  assert.equal(
    evaluateGate({ ...GOOD_ANSWERS, e1_tawhid: 'not_affirm' }, ADULT).result,
    'auto_reject'
  );
  assert.equal(
    evaluateGate({ ...GOOD_ANSWERS, e2_riba: 'not_affirm' }, ADULT).result,
    'auto_reject'
  );
  assert.equal(
    evaluateGate({ ...GOOD_ANSWERS, e3_ribaPractice: 'continuing' }, ADULT)
      .result,
    'auto_reject'
  );
});
...
test('E3 exiting still passes (honest-disclosure badge case)', () => {
  const v = evaluateGate({ ...GOOD_ANSWERS, e3_ribaPractice: 'exiting' }, ADULT);
  assert.equal(v.result, 'auto_pass');
});
...
test('missing structured answers escalate, never auto-pass', () => {
  ...
  const v = evaluateGate(withoutE1, ADULT);
  assert.equal(v.result, 'manual_review');
});
